...
class appnp(torch.nn.Module):
    def __init__(self, dataset, trial):
        ...
        for epoch in range(self.num_layers):
            self.dropout_list.append(trial.suggest_float(f'dropout_{epoch}', 0.1, 0.9))
    ...
    def __init__(self, dataset, trial):
        ...
        if self.num_layers >= 2:
            for n in range(self.num_layers -1):
                self.units_list.append(trial.suggest_int(f'n_units_{n}', 5, 200))
        ...
        for epoch in range(self.num_layers):
            ...
            if epoch == self.num_layers-1:
                ...
                out_features = self.units_list[epoch]
            self.layers_list.append(SplineConv(self.in_features,
                                               out_features,
                                               dim = 1,
